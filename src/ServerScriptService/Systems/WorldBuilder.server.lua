--[[
    WorldBuilder.server.lua — DataTycoon v0.20
    Dedicated world builder. Each section in pcall. task.wait() between sections.
    Glow toned way down. More map detail added.
]]

print("[WORLD] WorldBuilder v0.20 starting...")
...
local function Flower(x, z, folder)
    local palette = {"Bright red","Bright yellow","Bright violet","Bright orange","Pink","Cyan","Hot pink","Lavender"}
...
local function Lamp(x, z, folder)
    ...
    -- SmoothPlastic head (bright white but no bloom contribution)
...
-- Orb: toned-down glow. Ring = SmoothPlastic (no bloom). Sphere = small Neon.
local function Orb(cx, cy, cz, brickCol, folder)
    -- Ground ring — SmoothPlastic so it doesn't bloom, still collidable for Touched
    ...
    -- Floating sphere (Neon but small — subtle glow)
    ...
    bb.Size = UDim2.new(0,120,0,30); bb.StudsOffset=Vector3.new(0,4,0)
    bb.AlwaysOnTop=true; bb.Parent=anchor
    local lbl = Instance.new("TextLabel")
    lbl.Size=UDim2.new(1,0,1,0); lbl.BackgroundTransparency=1
    lbl.Text="✦ +5 Data"; lbl.TextColor3=brickCol.Color
...
local function section(name, fn)
    ...
    if not ok then
        warn("[WORLD] ERROR in "..name..": "..tostring(err))
    else
        print("[WORLD] "..name.." done")
    end
    task.wait()
end

-- ============================================================
-- LIGHTING (toned-down bloom)
-- ============================================================
section("Lighting", function()
    local lt = game:GetService("Lighting")
    lt.Ambient        = Color3.fromRGB(100,100,115)
    lt.OutdoorAmbient = Color3.fromRGB(120,120,135)
    lt.Brightness     = 2
    lt.ClockTime      = 14
    lt.FogEnd         = 1200
    lt.FogColor       = Color3.fromRGB(180,195,220)
    lt.GlobalShadows  = true

    -- Much more subtle bloom — was 0.5/35/0.7, now 0.1/16/0.88
    local bloom = Instance.new("BloomEffect")
    bloom.Intensity = 0.1; bloom.Size = 16; bloom.Threshold = 0.88
    bloom.Parent = lt

    local cc = Instance.new("ColorCorrectionEffect")
    cc.Brightness=0.02; cc.Contrast=0.06; cc.Saturation=0.15
    cc.TintColor=Color3.fromRGB(215,225,255); cc.Parent=lt

    local sr = Instance.new("SunRaysEffect")
    sr.Intensity=0.15; sr.Spread=0.8; sr.Parent=lt

    local atm = Instance.new("Atmosphere")
    atm.Density=0.2; atm.Offset=0.1; atm.Color=Color3.fromRGB(185,205,230)
    atm.Decay=0.9; atm.Glare=0.2; atm.Haze=1.2; atm.Parent=lt
end)
...
section("Hub", function()
    ...
    -- Decorative ring inlays
    P({name="HubInlay1", size=Vector3.new(60,0.12,60), pos=Vector3.new(0,3.07,0),
       color=BrickColor.new("Medium stone grey"), mat=Enum.Material.SmoothPlastic, parent=hub})
    P({name="HubInlay2", size=Vector3.new(30,0.12,30), pos=Vector3.new(0,3.07,0),
    ...
    for f=0,4 do
        ...
        P({name="Tower"..f, size=Vector3.new(sz,7,sz), pos=Vector3.new(0,y,0),
           color=col, mat=Enum.Material.Metal, parent=hub})
        -- SmoothPlastic cyan trim (no Neon bloom)
        P({name="Trim"..f, size=Vector3.new(sz+0.6,0.3,sz+0.6), pos=Vector3.new(0,y-3.4,0),
           color=BrickColor.new("Cyan"), mat=Enum.Material.SmoothPlastic, parent=hub})
    end

    -- FOUNTAIN at hub center (in front of tower base)
    P({name="FountainPool", size=Vector3.new(18,0.8,18), pos=Vector3.new(0,3.4,0),
       color=BrickColor.new("Dark stone grey"), mat=Enum.Material.SmoothPlastic,
       shape=Enum.PartType.Cylinder, parent=hub})
    -- Water surface
    local water = P({name="FountainWater", size=Vector3.new(16,0.3,16), pos=Vector3.new(0,3.9,0),
       color=BrickColor.new("Bright blue"), mat=Enum.Material.Glass,
       shape=Enum.PartType.Cylinder, alpha=0.3, collide=false, parent=hub})
    water.CFrame = CFrame.new(0,3.9,0) * CFrame.Angles(0,0,math.rad(90))
    -- Center spire
    P({name="FountainSpire", size=Vector3.new(1.5,8,1.5), pos=Vector3.new(0,7,0),
       color=BrickColor.new("Medium stone grey"), mat=Enum.Material.Metal, parent=hub})
    P({name="FountainTop", size=Vector3.new(4,1,4), pos=Vector3.new(0,11.5,0),
       color=BrickColor.new("Cyan"), mat=Enum.Material.SmoothPlastic,
       shape=Enum.PartType.Ball, collide=false, parent=hub})
    -- Fountain ring benches
    for i=1,6 do
        local a=(i/6)*PI*2; local r=10
        P({name="FBench", size=Vector3.new(4,0.5,1.5), pos=Vector3.new(math.cos(a)*r,3.3,math.sin(a)*r),
           color=BrickColor.new("Brown"), mat=Enum.Material.Wood, rot=math.deg(a)+90, parent=hub})
    end

    -- Server racks
    for i=1,8 do
        local a=(i/8)*PI*2; local x,z=math.cos(a)*30,math.sin(a)*30
        P({name="Rack"..i, size=Vector3.new(5,11,3), pos=Vector3.new(x,7,z),
           color=BrickColor.new("Dark stone grey"), mat=Enum.Material.Metal, parent=hub})
        -- LED strips (SmoothPlastic, not Neon)
        for l=0,2 do
            local col2 = l%2==0 and BrickColor.new("Lime green") or BrickColor.new("Forest green")
            P({name="LED", size=Vector3.new(0.3,0.2,0.3), pos=Vector3.new(x,3+l*2.5,z+1.6),
               color=col2, mat=Enum.Material.SmoothPlastic, parent=hub})
        end
    end
    ...
    -- Raised plazas connecting hub to walkways
    for _, d in ipairs({{0,1},{0,-1},{1,0},{-1,0}}) do
        local dx,dz=d[1],d[2]
        P({name="Plaza", size=Vector3.new(dx~=0 and 30 or 8, 0.3, dz~=0 and 30 or 8),
           pos=Vector3.new(dx*57, 0.15, dz*57),
           color=BrickColor.new("Medium stone grey"), mat=Enum.Material.Concrete, parent=hub})
    end

    -- Flowers ringing the hub platform edge
    for i=1,20 do
        local a=(i/20)*PI*2; local r=40+R(-2,2)
...
section("Walkways", function()
    ...
    for di, d in ipairs({{0,1},{0,-1},{1,0},{-1,0}}) do
        ...
        if dx~=0 then
            ...
            -- Edge stripes
            P({name="WalkStripeL"..di, size=Vector3.new(230,0.32,0.4), pos=Vector3.new(mx,0.15,mz+3.8), color=BrickColor.new("Smoky grey"), mat=wM, parent=walk})
            P({name="WalkStripeR"..di, size=Vector3.new(230,0.32,0.4), pos=Vector3.new(mx,0.15,mz-3.8), color=BrickColor.new("Smoky grey"), mat=wM, parent=walk})
        else
            P({name="Walk"..di, size=Vector3.new(8,0.3,230), pos=Vector3.new(mx,0.15,mz), color=wC, mat=wM, parent=walk})
            P({name="WalkStripeL"..di, size=Vector3.new(0.4,0.32,230), pos=Vector3.new(mx+3.8,0.15,mz), color=BrickColor.new("Smoky grey"), mat=wM, parent=walk})
            P({name="WalkStripeR"..di, size=Vector3.new(0.4,0.32,230), pos=Vector3.new(mx-3.8,0.15,mz), color=BrickColor.new("Smoky grey"), mat=wM, parent=walk})
        end
        ...
        for dist=90,230,55 do
            ...
            if dx==0 then
                ...
                P({name="BenchBack",size=Vector3.new(5,2.5,0.4),pos=Vector3.new(bx+12,3.5,bz+0.7),color=BrickColor.new("Brown"),mat=Enum.Material.Wood,parent=walk})
                P({name="Bench",size=Vector3.new(5,0.5,1.8),pos=Vector3.new(bx-12,2,bz),color=BrickColor.new("Brown"),mat=Enum.Material.Wood,parent=walk})
                P({name="BenchBack",size=Vector3.new(5,2.5,0.4),pos=Vector3.new(bx-12,3.5,bz+0.7),color=BrickColor.new("Brown"),mat=Enum.Material.Wood,parent=walk})
            else
                ...
                P({name="BenchBack",size=Vector3.new(0.4,2.5,5),pos=Vector3.new(bx+0.7,3.5,bz+12),color=BrickColor.new("Brown"),mat=Enum.Material.Wood,parent=walk})
                P({name="Bench",size=Vector3.new(1.8,0.5,5),pos=Vector3.new(bx,2,bz-12),color=BrickColor.new("Brown"),mat=Enum.Material.Wood,parent=walk})
                P({name="BenchBack",size=Vector3.new(0.4,2.5,5),pos=Vector3.new(bx+0.7,3.5,bz-12),color=BrickColor.new("Brown"),mat=Enum.Material.Wood,parent=walk})
            end
        end

        -- Small data kiosks every 80 studs
        for dist=80,220,80 do
            local kx,kz=dx*dist,dz*dist
            local offX = dx==0 and 14 or 0; local offZ = dz==0 and 14 or 0
            P({name="Kiosk",size=Vector3.new(3,5,3),pos=Vector3.new(kx+offX,2.5,kz+offZ),color=BrickColor.new("Medium stone grey"),mat=Enum.Material.SmoothPlastic,parent=walk})
            P({name="KioskTop",size=Vector3.new(3.5,0.4,3.5),pos=Vector3.new(kx+offX,5.2,kz+offZ),color=BrickColor.new("Cyan"),mat=Enum.Material.SmoothPlastic,parent=walk})
            P({name="KioskScreen",size=Vector3.new(2,1.5,0.2),pos=Vector3.new(kx+offX,3.5,kz+offZ+(dz~=0 and 1.6 or 0)),color=BrickColor.new("Bright blue"),mat=Enum.Material.SmoothPlastic,parent=walk})
        end

        -- Flowers along walkway edges
        for dist=50,250,35 do
            local fx,fz=dx*dist,dz*dist
            if dx==0 then
                Flower(fx+11+R(-1,1),fz+R(-3,3),walk)
                Flower(fx-11+R(-1,1),fz+R(-3,3),walk)
            else
                Flower(fx+R(-3,3),fz+11+R(-1,1),walk)
                Flower(fx+R(-3,3),fz-11+R(-1,1),walk)
            end
        end
    end
...
section("Plots", function()
    ...
    for _, pp in ipairs(coords) do
        ...
        -- Ground slab
        P({name="Plot_"..x.."_"..z, size=Vector3.new(116,0.6,116),
           pos=Vector3.new(cx,0.3,cz), color=BrickColor.new("Dark green"),
           mat=Enum.Material.SmoothPlastic, parent=pf})

        -- Low border wall (full perimeter)
        P({name="BorderN",size=Vector3.new(116,2,0.8),pos=Vector3.new(cx,1.6,cz+58),color=BrickColor.new("Dark stone grey"),mat=Enum.Material.Concrete,parent=pf})
        P({name="BorderS",size=Vector3.new(116,2,0.8),pos=Vector3.new(cx,1.6,cz-58),color=BrickColor.new("Dark stone grey"),mat=Enum.Material.Concrete,parent=pf})
        P({name="BorderE",size=Vector3.new(0.8,2,116),pos=Vector3.new(cx+58,1.6,cz),color=BrickColor.new("Dark stone grey"),mat=Enum.Material.Concrete,parent=pf})
        P({name="BorderW",size=Vector3.new(0.8,2,116),pos=Vector3.new(cx-58,1.6,cz),color=BrickColor.new("Dark stone grey"),mat=Enum.Material.Concrete,parent=pf})

        -- Corner posts
        for _, c in ipairs({{57,57},{-57,57},{57,-57},{-57,-57}}) do
            P({name="Post",size=Vector3.new(1.5,6,1.5),pos=Vector3.new(cx+c[1],3,cz+c[2]),
               color=BrickColor.new("Medium stone grey"),mat=Enum.Material.SmoothPlastic,parent=pf})
        end
        ...
        -- Billboard
        local anchor = P({name="SignBB",size=Vector3.new(0.1,0.1,0.1),
            pos=Vector3.new(cx,9.5,cz),alpha=1,collide=false,parent=pf})
        local bb = Instance.new("BillboardGui")
        bb.Size=UDim2.new(0,160,0,70); bb.StudsOffset=Vector3.new(0,3,0)
        bb.AlwaysOnTop=false; bb.Parent=anchor
        local t1=Instance.new("TextLabel"); t1.Size=UDim2.new(1,0,0.45,0)
        t1.BackgroundTransparency=1; t1.Text="💰 "..price.." Data"
        t1.TextColor3=Color3.fromRGB(255,220,100); t1.TextSize=15
        t1.Font=Enum.Font.GothamBold; t1.TextStrokeTransparency=0.35; t1.Parent=bb
        local t2=Instance.new("TextLabel"); t2.Size=UDim2.new(1,0,0.4,0)
        t2.Position=UDim2.new(0,0,0.5,0); t2.BackgroundTransparency=1
        t2.Text="🌿 AVAILABLE"; t2.TextColor3=Color3.fromRGB(100,255,150)
        t2.TextSize=12; t2.Font=Enum.Font.GothamBold; t2.TextStrokeTransparency=0.5; t2.Parent=bb
        ...
        -- Entrance path from border inward
        P({name="EntryPath",size=Vector3.new(8,0.3,20),pos=Vector3.new(cx,0.5,cz+48),
           color=BrickColor.new("Light stone grey"),mat=Enum.Material.Concrete,parent=pf})
    end
end)

-- ============================================================
-- DATA ORBS (toned-down glow)
-- ============================================================
...
section("Nature", function()
    ...
    -- Corner forests
    for _, fc in ipairs({{-210,-210},{210,-210},{-210,210},{210,210}}) do
    ...
    -- Mid-zone trees (between hub and plots)
    for i=1,45 do
        local tx,tz=R(-230,230),R(-230,230)
        if math.abs(tx)>50 or math.abs(tz)>50 then
            Tree(tx,tz,0.5+R()*0.4,nat)
        end
    end
    task.wait()
    ...
    -- Small ponds
    for _, pc in ipairs({{65,65},{-65,-65},{65,-65},{-65,65}}) do
        local pond = P({name="Pond",size=Vector3.new(18,0.4,12),pos=Vector3.new(pc[1],0.2,pc[2]),
            color=BrickColor.new("Bright blue"),mat=Enum.Material.Glass,
            shape=Enum.PartType.Cylinder,alpha=0.35,collide=false,parent=nat})
        pond.CFrame = CFrame.new(pc[1],0.2,pc[2]) * CFrame.Angles(0,0,math.rad(90))
        for j=1,5 do
            Rock(pc[1]+math.cos((j/5)*PI*2)*10, pc[2]+math.sin((j/5)*PI*2)*7, 0.3, nat)
        end
    end
end)
...
section("Buildings", function()
    ...
    for _, b in ipairs({
        ...
        -- Roof accent (SmoothPlastic, not Neon)
        ...
        -- Windows
        for w=-1,1,2 do
            P({name="Window",size=Vector3.new(3,3,0.3),pos=Vector3.new(b[1]+w*4,8,b[2]+8.15),color=BrickColor.new("Bright blue"),mat=Enum.Material.Glass,alpha=0.3,parent=bf})
        end
        -- Sign billboard
        ...
        bb.StudsOffset=Vector3.new(0,1.5,0); bb.AlwaysOnTop=true; bb.Parent=sign
        local lbl=Instance.new("TextLabel"); lbl.Size=UDim2.new(1,0,1,0)
        lbl.BackgroundTransparency=1; lbl.Text="🏪 "..b[3]
        lbl.TextColor3=Color3.fromRGB(255,255,255); lbl.TextSize=13
        lbl.Font=Enum.Font.GothamBold; lbl.TextStrokeTransparency=0.3; lbl.Parent=bb
        -- A lamp outside each building
